...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
			...
			$this->design->smarty->registerPlugin("function", "get_posts_news",          array($this, 'get_posts_news_plugin'));
			$this->design->smarty->registerPlugin("function", "get_posts_blog",          array($this, 'get_posts_blog_plugin'));
     ...
     *
     */
    public function get_posts_news_plugin($params, &$smarty)
    {
        if(!isset($params['visible']))
            $params['visible'] = 1;
     ...
     *
     */
    public function get_posts_blog_plugin($params, &$smarty)
    {
        if(!isset($params['visible']))
            $params['visible'] = 1;
        if(!isset($params['news']))
    ...
    public function get_comments_plugin($params, &$smarty)
    {
        ...
        if(!isset($params['object_id']))
            $params['object_id'] = 4;
        if(!empty($params['var']))
            $smarty->assign($params['var'], $this->comments->count_comments($params));
    }
