...
class PostAdmin extends Simpla
{
	...
	public function fetch()
	{
		if($this->request->method('post'))
		{
			...
			$post->date = date('Y-m-d-H-i', strtotime($this->request->post('date')));
			
			$post->visible = $this->request->post('visible', 'boolean');
            $post->news = $this->request->post('news', 'boolean');
			...
			else
			{
                ...
                {
                    $this->blog->delete_image($post->id);
                    move_uploaded_file($image['tmp_name'], $this->root_dir.$this->config->news_images_dir.$image['name']);
