...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div id="list">
			...
			<div class="{if !$post->visible}invisible{/if} {if !$post->news}notnews{/if} row">
				...
				<div class="name cell">		
					...
					<br>
					{$post->date|date|date_format:"%d.%m.%Y"}
				</div>
...
$(function() {
    ...
    $("a.news").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax/update_object.php',
            data: {'object': 'blog', 'id': id, 'values': {'news': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
            success: function(data){
                icon.removeClass('loading_icon');
                if(state)
