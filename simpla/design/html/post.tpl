...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	<div id="name">
		...
		<div class="checkbox">
            ...
            <input name="news" value='1' type="checkbox" id="news_checkbox" {if $post->news}checked{/if}/> <label for="news_checkbox">Новость?</label>
		</div>
	...
	<div id="column_left">
		...
		<div class="block">
			<ul>
				<li><label class=property>Дата</label><input type=text name=date value='{$post->date|date|date_format:"%d.%m.%Y"}'></li>
			</ul>
		</div>
	...
	<div id="column_right">
        ...
        <div class="block layer images">
            ...
            <input class='upload_image' name=image type=file value="../{$config->news_images_dir}{$post->image}" />

            {if $post->image}
                <ul>
                    <li>
                        <a href='#' class="delete"><img src='design/images/cross-circle-frame.png'></a>
                        <img src="../{$config->news_images_dir}{$post->image}" alt="" />
                    </li>
                </ul>
            {/if}
        </div>
