...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div class="{if !$product->visible}invisible{/if} {if $product->featured}featured{/if} {if $product->notbuy}notbuy{/if} row">
...
$(function() {
    ...
    $("a.notbuy").click(function() {
        ...
        icon.addClass('loading_icon');
            ...
            type: 'POST',
            url: 'ajax/update_object.php',
            data: {'object': 'product', 'id': id, 'values': {'notbuy': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
            success: function(data){
                icon.removeClass('loading_icon');
                if(state)
