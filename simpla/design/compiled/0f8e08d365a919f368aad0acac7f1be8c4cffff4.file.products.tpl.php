<?php /* Smarty version Smarty-3.0.7, created on 2012-12-28 17:49:44
         compiled from "simpla/design/html\products.tpl" */ ?>
<?php /*%%SmartyHeaderCode:2288950ddb18864a6c9-00379953%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '0f8e08d365a919f368aad0acac7f1be8c4cffff4' => 
    array (
  ...
  'nocache_hash' => '2288950ddb18864a6c9-00379953',
  'function' => 
  array (
    'category_select' => 
    array (
...
if (!is_callable('smarty_modifier_truncate')) include 'Y:\domains\oc\Smarty\libs\plugins\modifier.truncate.php';
?>
<?php ob_start(); ?>
...
?>
		<div class="<?php if (!$_smarty_tpl->getVariable('product')->value->visible){?>invisible<?php }?> <?php if ($_smarty_tpl->getVariable('product')->value->featured){?>featured<?php }?> <?php if ($_smarty_tpl->getVariable('product')->value->notbuy){?>notbuy<?php }?> row">
...
"><?php echo smarty_modifier_escape($_smarty_tpl->getVariable('product')->value->name);?>
			...
			<div class="icons cell">
				<a class="preview"   title="Предпросмотр в новом окне" href="../products/<?php echo $_smarty_tpl->getVariable('product')->value->url;?>
" target="_blank"></a>			
				<a class="enable"    title="Активен"                 href="#"></a>
				<a class="featured"  title="Рекомендуемый"           href="#"></a>
                <a class="notbuy"    title="Покупаймый"              href="#"></a>
		...
		<div id="action">
			...
			<select name="action">
                ...
                <option value="set_notbuy">Нельзя купить</option>
                <option value="unset_notbuy">Отменить НЕ покупаймость</option>
...
'},
    ...
    // Сделать хитом
    $("a.notbuy").click(function() {
        var icon        = $(this);
        var line        = icon.closest("div.row");
        var id          = line.find('input[type="checkbox"][name*="check"]').val();
        var state       = line.hasClass('notbuy')?0:1;
        icon.addClass('loading_icon');
            $.ajax({
            type: 'POST',
            url: 'ajax/update_object.php',
            data: {'object': 'product', 'id': id, 'values': {'notbuy': state}, 'session_id': '<?php echo $_SESSION['id'];?>
'},
            success: function(data){
                icon.removeClass('loading_icon');
                if(state)
                    line.addClass('notbuy');
                else
                    line.removeClass('notbuy');
            },
            dataType: 'json'
        });
        return false;
