<?php /* Smarty version Smarty-3.0.7, created on 2013-01-11 17:55:53
         compiled from "simpla/design/html\blog.tpl" */ ?>
<?php /*%%SmartyHeaderCode:2663350f027f948ab31-12970237%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '9ded5c8dbc29bd2d78a335176d942fc910e5de65' => 
    array (
  ...
  'nocache_hash' => '2663350f027f948ab31-12970237',
  'function' => 
  array (
  ),
...
?>
			<div class="<?php if (!$_smarty_tpl->getVariable('post')->value->visible){?>invisible<?php }?> <?php if (!$_smarty_tpl->getVariable('post')->value->news){?>notnews<?php }?> row">
...
"><?php echo smarty_modifier_escape($_smarty_tpl->getVariable('post')->value->name);?>
				...
				<div class="icons cell">
					<a class="preview" title="Предпросмотр в новом окне" href="../blog/<?php echo $_smarty_tpl->getVariable('post')->value->url;?>
" target="_blank"></a>
                    <a class="news" title="Новость" href="#"></a>
		...
		<span id="select">
		<select name="action">
            <option value="news">Сделать новостью</option>
            <option value="notnews">Обычная блогозапись</option>
...
'},
    ...
    $("a.news").click(function() {
        ...
        var line        = icon.closest(".row");
        var id          = line.find('input[type="checkbox"][name*="check"]').val();
        var state       = line.hasClass('notnews')?1:0;
        icon.addClass('loading_icon');
        $.ajax({
            type: 'POST',
            url: 'ajax/update_object.php',
            data: {'object': 'blog', 'id': id, 'values': {'news': state}, 'session_id': '<?php echo $_SESSION['id'];?>
'},
            success: function(data){
                icon.removeClass('loading_icon');
                if(state)
                    line.removeClass('notnews');
                else
                    line.addClass('notnews');
            },
            dataType: 'json'
        });
            return false;
