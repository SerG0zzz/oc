...
class Blog extends Simpla
{
	...
	*/
	public function get_post($id)
	{
		...
		$query = $this->db->placehold("SELECT b.id, b.url, b.name, b.annotation, b.text, b.meta_title,
		                               b.meta_keywords, b.meta_description, b.visible, b.date, b.news, b.image
	...
	*/
	...
	{	
        ...
        if(isset($filter['news']))
            $news_filter = $this->db->placehold('AND b.news = ?', intval($filter['news']));
		...
		$query = $this->db->placehold("SELECT b.id, b.url, b.name, b.annotation, b.text,
		                                      ...
		                                      b.date, b.news, b.image
		                                      FROM __blog b WHERE 1 $post_id_filter $visible_filter $keyword_filter $news_filter
	...
	*/
	...
	{	
        ...
        if(isset($filter['news']))
            $visible_filter = $this->db->placehold('AND b.news = ?', intval($filter['news']));
	...
	*/	
    ...
    /*
	*
	* Удаление изображения анонса новости
	* @param $id
	*
	*/
    public function delete_image($blog_id)
    {
        $query = $this->db->placehold("SELECT image FROM __blog WHERE id=?", intval($blog_id));
        $this->db->query($query);
        $filename = $this->db->result('image');
        if(!empty($filename))
        {
            $query = $this->db->placehold("UPDATE __blog SET image=NULL WHERE id=?", $blog_id);
            $this->db->query($query);
            ...
            if($count == 0)
            {
                @unlink($this->config->root_dir.$this->config->news_images_dir.$filename);
            }
        }
    }
}
